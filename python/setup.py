...
def install_agent(vmx_path, site_token):
    ...
    if ip in ("192.168.192.10", "192.168.192.20", "192.168.192.21"):
        ...
        run_script(
            ...
            password=password,
            script=(
                f"SCHTASKS /create /tn Agent /sc once /tr \"msiexec /i 'C:\\Users\\jeanluc\\Desktop\\SentinelInstaller_windows_64bit.msi' /Q SITE_TOKEN={site_token}\" /ru interactive /rl highest /st 00:00 /f && SCHTASKS /run /tn Agent && SCHTASKS /delete /tn Agent /f"
            ),
        )
